...
def get_missing_skills(resume_text: str, job_description: str) -> list:
    ...
    missing = [s for s in tech_skills if s in jd_lower and s not in resume_lower]
    return missing[:8]
...
def run_analyst(jobs: list, resume_text: str, decode_jds: bool = False) -> list:
    ...
    for job in jobs:
        ...
        salary      = estimate_salary(job.get("title", ""), job.get("location", ""))

        job["fit_score"]      = fit_score
        job["missing_skills"] = missing
        job["ats_score"]      = ats["score"]
        job["salary"]         = salary
        job["jd_decoded"]     = decode_jd(job["title"], description) if decode_jds else "Click to decode"
        job["keywords"]       = missing[:6]

        analyzed.append(job)

    analyzed.sort(key=lambda x: x["fit_score"], reverse=True)
    top_score = analyzed[0]["fit_score"] if analyzed else 0
    print(f"✅ Analysis complete. Top fit: {top_score}%\n")
    return analyzed
