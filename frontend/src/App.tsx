  ...
  return (
    <Router>
      <div style={{display:'flex',height:'100vh',background:'#060B18',position:'relative',overflow:'hidden'}}>
        ...
        {/* Sidebar */}
        <aside style={{width:220,background:'rgba(255,255,255,0.03)',borderRight:'1px solid rgba(255,255,255,0.06)',display:'flex',flexDirection:'column',position:'relative',zIndex:10,flexShrink:0}}>

          {/* Logo */}
          <div style={{padding:'24px 20px 20px',borderBottom:'1px solid rgba(255,255,255,0.06)'}}>
            <div style={{width:38,height:38,background:'linear-gradient(135deg,#2563EB,#06B6D4)',borderRadius:10,display:'flex',alignItems:'center',justifyContent:'center',fontSize:20,marginBottom:10}}>🤖</div>
          ...
          <div style={{padding:'12px 14px',borderTop:'1px solid rgba(255,255,255,0.06)'}}>
            ...
            {agentStatus.map(a=>(
              <div key={a.name} style={{display:'flex',alignItems:'center',gap:7,marginBottom:5}}>
                <div style={{width:7,height:7,borderRadius:'50%',background:a.color,boxShadow:a.active?`0 0 6px ${a.color}`:'none',animation:a.active?'pulse 1.5s ease-in-out infinite':'none',flexShrink:0}}/>
          ...
          <div style={{padding:'10px 14px',borderTop:'1px solid rgba(255,255,255,0.06)',fontSize:11,color:'rgba(255,255,255,0.3)'}}>
            {time.toLocaleTimeString('en-US',{hour:'2-digit',minute:'2-digit'})} · {time.toLocaleDateString('en-US',{month:'short',day:'numeric',year:'numeric'})}
          </div>
        </aside>
