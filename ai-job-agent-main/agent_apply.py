import re, os, csv, time, yaml, pathlib, requests, urllib3
from bs4 import BeautifulSoup
from playwright.sync_api import sync_playwright
from docx import Document
from nltk.tokenize import wordpunct_tokenize

# Disable SSL warnings for college network
...
def fetch_greenhouse(org):
    ...
    try:
        j = requests.get(url, timeout=20, verify=False).json().get("jobs", [])
        out=[]
        for x in j:
            out.append({
                "source":"Greenhouse",
        ...
        print(f"Greenhouse error for {org}: {e}")
        return []

def fetch_lever(org):
    url = f"https://api.lever.co/v0/postings/{org}?mode=json"
    try:
        posts = requests.get(url, timeout=20, verify=False).json()
        out=[]
        for p in posts:
...
def open_and_prefill(url, ident, resume_path, cover_path=None, headless=False, pause_ms=120000):
    try:
        with sync_playwright() as p:
            ...
            page.bring_to_front()
            print("✅ Prefilled. Review and click Submit yourself.")
...
def main(top=5):
    ...
    print("🔍 Fetching jobs from Greenhouse and Lever...")
    jobs=[]
    for org in CFG["sources"]["greenhouse"]:
        fetched = fetch_greenhouse(org)
        print(f"  Greenhouse {org}: {len(fetched)} jobs")
        jobs += fetched
    for org in CFG["sources"]["lever"]:
        fetched = fetch_lever(org)
        print(f"  Lever {org}: {len(fetched)} jobs")
        jobs += fetched
    ...
    print(f"✅ Matched {len(matches)} jobs after filtering. Taking top {min(len(Q), top)}.\n")
    ...
    if not Q:
        print("❌ No matching jobs found. Try updating filters in config.yaml")
        return
    ...
    with open(log_path, "a", newline="", encoding="utf-8") as f:
        ...
        for j in Q[:top]:
            print(f"📄 Processing: {j['title']} at {j['org']}")
            ...
            print(f"  ✅ Resume saved: {resume_out.name}")

            doc = Document()
            doc.add_paragraph("Dear Hiring Team,")
            doc.add_paragraph(
                f"I'm applying for {j['title']} at {j['org']}. "
                f"My experience in data science and machine learning includes projects like "
                f"{projects[0]['title'] if projects else 'NLP and ML pipelines'}."
            )
            ...
            print(f"  ✅ Cover letter saved: {cover_out.name}")

            open_and_prefill(
                j["url"], CFG["identity"], str(resume_out), str(cover_out),
    ...
    print("🎉 Done! Check the 'out' folder for resumes and cover letters.")
